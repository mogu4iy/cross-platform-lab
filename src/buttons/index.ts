...
export const backButton = (ctx: SceneContext, hide: boolean = false): HideableKBtn => {
  return Markup.button.text(emoji.emojify(ctx.session.i18n.__(`button:back`)), hide);
};

export const infoButton = (ctx: SceneContext, hide: boolean = false): HideableKBtn => {
  return Markup.button.text(emoji.emojify(ctx.session.i18n.__(`button:info`)), hide);
};

export const manageSubscriptionButton = (ctx: SceneContext, hide: boolean = false): HideableKBtn => {
  return Markup.button.text(emoji.emojify(ctx.session.i18n.__(`button:manage_subscription`)), hide);
};

export const manageChannelButton = (ctx: SceneContext, hide: boolean = false): HideableKBtn => {
  return Markup.button.text(emoji.emojify(ctx.session.i18n.__(`button:manage_channel`)), hide);
};
