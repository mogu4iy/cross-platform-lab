...
const enterPromiseHandler: TPromiseHandlerType<SceneContext> = async ({ data, ctx, next }) => {
  ...
  const keyboardMessage = await ctx.reply(ctx.session.i18n.__('keyboards.main_keyboard.info'), mainKeyboard(ctx));
...
const messagePromiseHandler: TPromiseHandlerType<SceneContext> = async ({ data, ctx, next }) => {
  if ('message' in ctx.update) {
    ...
    if ('text' in ctx.update.message) {
      Console.log(ctx.update.message.text, ctx.update.message.message_id, '   INFO');
      switch (ctx.update.message.text) {
        case ctx.session.i18n.__(`keyboards.back_keyboard.back`):
          const sceneHistory = ctx.session.__scenes.state.sceneHistory;
          const backScene = sceneHistory.length > 0 ? sceneHistory[sceneHistory.length - 1] : config.TELEGRAM.SCENE.INFO;
